...
export default function DashboardPage() {
  ...
  const [candidateSessions, setCandidateSessions] = useState<CandidateSessionInfo[]>([]);
  const [loadingCandidates, setLoadingCandidates] = useState(false);

  // Candidate Report Viewer Modal
  const [selectedReportSession, setSelectedReportSession] = useState<CandidateSessionInfo | null>(null);
  ...
  const handleCreateInterview = async (e: React.FormEvent) => {
    ...
    if (!title.trim() || !role.trim()) {
      setCreateError('Please fill in Title and Role');
      return;
    }
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setCreateError('Failed to create interview');
      }
    } finally {
      setCreating(false);
    }
  ...
  const copyShareLink = (shareToken: string) => {
    ...
    const candidateUrl = `${origin}/interview/${shareToken}`;
    navigator.clipboard.writeText(candidateUrl);
    setCopiedToken(shareToken);
    showToast('Candidate share link copied to clipboard!');
    setTimeout(() => setCopiedToken(null), 2500);
  };

  const handleViewCandidates = async (interview: Interview) => {
    setSelectedInterview(interview);
    setLoadingCandidates(true);
    try {
      const sessions = await getCandidateSessionsForInterview(interview.id);
  ...
  const handleViewReport = async (session: CandidateSessionInfo) => {
    setSelectedReportSession(session);
    setLoadingReport(true);
    setSelectedReport(null);

    try {
      const reportData = await getCandidateReportBySessionId(session.id);
  ...
  const handleDeleteInterview = async (interview: Interview) => {
    if (!window.confirm(`Are you sure you want to delete "${interview.title}"?`)) return;
    ...
    try {
      ...
      if (err instanceof Error) {
        showToast(`Failed to delete interview: ${err.message}`);
      } else {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div className="loading-container">
          ...
          <p className="pulse-text">Loading Recruiter Dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--background)' }}>
      ...
      <header className="brand-header">
        <div className="brand-logo" style={{ cursor: 'pointer' }} onClick={() => router.push('/dashboard')}>
      ...
      <main style={{ flex: 1, padding: '2.5rem 1.5rem', maxWidth: '1100px', margin: '0 auto', width: '100%' }}>
        ...
        >
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: '0.2rem' }}>
              Create AI technical interviews and manage shareable candidate links.
            </p>
          </div>
        ...
        {loadingInterviews ? (
          <div className="loading-container">
            <div className="spinner"></div>
            <p className="pulse-text">Loading your active interviews...</p>
          </div>
        ) : interviews.length === 0 ? (
          <div className="glass-card" style={{ textAlign: 'center', padding: '4rem 2rem' }}>
            <Briefcase size={44} color="var(--text-dim)" style={{ marginBottom: '1rem' }} />
            <h3 style={{ fontSize: '1.25rem', fontWeight: 600, marginBottom: '0.5rem', color: 'var(--text-main)' }}>
              No Interviews Created Yet
            </h3>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', marginBottom: '1.5rem' }}>
              Get started by creating your first automated AI technical interview session.
            </p>
            <button
              onClick={() => setShowCreateModal(true)}
              className="btn-primary"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem' }}>
            {interviews.map((interview) => {
              ...
              return (
                <div key={interview.id} className="glass-card" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                    <span className="badge" style={{ textTransform: 'capitalize' }}>
                      {interview.difficulty}
                    </span>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                      {new Date(interview.created_at || Date.now()).toLocaleDateString()}
                    </span>
                  </div>

                  <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '0.35rem', color: 'var(--text-main)' }}>
                    {interview.title}
                  </h3>
                  ...
                  <div style={{ marginTop: 'auto' }}>
                    ...
                    >
                      ...
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                        <BarChart2 size={14} /> Status: {interview.status}
                      </span>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button
                        onClick={() => copyShareLink(interview.share_token)}
                        style={{
                          flex: 1,
                          background: copiedToken === interview.share_token ? '#ecfdf5' : '#eef2ff',
                          border: copiedToken === interview.share_token ? '1px solid #a7f3d0' : '1px solid #c7d2fe',
                          color: copiedToken === interview.share_token ? '#047857' : '#3730a3',
                          padding: '0.6rem',
                          borderRadius: '6px',
                          cursor: 'pointer',
                          display: 'flex',
                      ...
                      >
                        {copiedToken === interview.share_token ? (
                          <>
                      ...
                      <button
                        onClick={() => handleViewCandidates(interview)}
                        style={{
                          background: '#ffffff',
                          border: '1px solid var(--border)',
                          color: 'var(--text-main)',
        ...
        >
          <div className="glass-card" style={{ maxWidth: '540px', width: '100%', padding: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ fontSize: '1.35rem', fontWeight: 700, color: 'var(--text-main)' }}>Create AI Technical Interview</h2>
              <button
            ...
            <form onSubmit={handleCreateInterview}>
              <div className="form-group">
                <label className="form-label" htmlFor="interview-title">
                  Interview Title *
                </label>
                <input
                  id="interview-title"
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="interview-role">
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="interview-desc">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="interview-duration">
                    Duration (Minutes)
                  </label>
                  <input
      ...
      {selectedInterview && (
        ...
        >
          <div className="glass-card" style={{ maxWidth: '640px', width: '100%', padding: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <div>
                <h2 style={{ fontSize: '1.3rem', fontWeight: 700, color: 'var(--text-main)' }}>Candidate Submissions</h2>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>{selectedInterview.title}</p>
              </div>
              <button
            ...
            {loadingCandidates ? (
              <div className="loading-container" style={{ padding: '2rem' }}>
                <div className="spinner"></div>
                <p className="pulse-text">Loading candidate records...</p>
              </div>
            ) : candidateSessions.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', textAlign: 'center', padding: '2rem 0' }}>
                No candidates have started this interview session yet. Share the link to receive submissions.
              </p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxHeight: '400px', overflowY: 'auto' }}>
                {candidateSessions.map((session) => (
                  ...
                  >
                    <div>
                      <div style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text-main)' }}>{session.name}</div>
      ...
      {/* Candidate AI Report Viewer Modal */}
      {selectedReportSession && (
        ...
        >
          <div className="glass-card" style={{ maxWidth: '680px', width: '100%', padding: '2rem', maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <div>
                <h2 style={{ fontSize: '1.3rem', fontWeight: 700, color: 'var(--text-main)' }}>
                  AI Assessment Report: {selectedReportSession.name}
                </h2>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>{selectedReportSession.email}</p>
              </div>
              <button
                onClick={() => {
            ...
            {loadingReport ? (
              <div className="loading-container" style={{ padding: '2rem' }}>
                <div className="spinner"></div>
                <p className="pulse-text">Fetching AI evaluation report...</p>
              </div>
            ) : !selectedReport ? (
              <div style={{ textAlign: 'center', padding: '2rem' }}>
                <p style={{ color: 'var(--text-muted)' }}>
                  Report not available yet. The report is generated when the candidate completes all questions or the session expires.
                </p>
              </div>
            ) : (
              <div style={{ background: '#f8fafc', border: '1px solid var(--border)', padding: '1.5rem', borderRadius: '10px' }}>
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--primary)' }}>
                      {selectedReport.overall_score.toFixed(1)} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>/ 100</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <span
                      ...
                      style={{
                        ...
                        background:
                          selectedReport.recommendation === 'strong_hire' || selectedReport.recommendation === 'hire'
                            ? '#ecfdf5'
                            : selectedReport.recommendation === 'maybe'
                            ? '#fefce8'
                            : '#fef2f2',
                        color:
                          selectedReport.recommendation === 'strong_hire' || selectedReport.recommendation === 'hire'
                  ...
                  {selectedReport.strengths && selectedReport.strengths.length > 0 && (
                    <div style={{ background: '#ffffff', border: '1px solid var(--border)', padding: '0.875rem', borderRadius: '8px' }}>
                      <div style={{ fontWeight: 600, fontSize: '0.85rem', color: '#047857', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                        <ThumbsUp size={14} /> Demonstrated Strengths
                      </div>
                ...
                {selectedReport.skills && selectedReport.skills.length > 0 && (
                  <div>
                    <div style={{ fontWeight: 600, fontSize: '0.9rem', marginBottom: '0.5rem', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                      <BarChart size={16} color="var(--primary)" /> Technical Competency Breakdown
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                      {selectedReport.skills.map((sk, idx) => (
                        <div key={idx} style={{ background: '#ffffff', border: '1px solid var(--border)', padding: '0.69rem 0.85rem', borderRadius: '6px', fontSize: '0.8rem' }}>
